...
(function() {
	...
	var c = function(a, b) {
		...
		for ( var m = 0; m < a.length; m += 16) {
			...
			for ( var s = 0; s < 80; s++) {
				s < 16 ? c[s] = a[m + s] : c[s] = g(c[s - 3]
						^ c[s - 8] ^ c[s - 14] ^ c[s - 16], 1);
				var t = f(f(g(h, 5), d(s, i, j, k)), f(f(l, c[s]),
						e(s)));
				l = k;
				k = j;
				j = g(i, 30);
...
(function() {
	...
	function K(a) {
		return a.s < 0 || a.compareTo(this.m) >= 0 ? a.mod(this.m)
				: a
	}
	function J(a) {
		this.m = a
	...
	function H(a, b, c) {
		...
		if (!(f.t <= 0)) {
			...
			c == null && (c = e());
			var h = e(), i = this.s, j = a.s, k = this.DB
					- y(f[f.t - 1]);
			if (k > 0) {
				f.lShiftTo(k, h);
				g.lShiftTo(k, c)
			...
			if (m == 0)
				return;
			var n = m * (1 << this.F1)
					+ (l > 1 ? h[l - 2] >> this.F2 : 0), o = this.FV
					/ n, p = (1 << this.F1) / n, q = 1 << this.F2, r = c.t, s = r
					- l, t = b == null ? e() : b;
			h.dlShiftTo(s, t);
			if (c.compareTo(t) >= 0) {
				c[c.t++] = 1;
			...
			while (h.t < l)
				h[h.t++] = 0;
			while (--s >= 0) {
				var u = c[--r] == m ? this.DM : Math.floor(c[r] * o
						+ (c[r - 1] + q) * p);
	...
	function G(a) {
		...
		for (c = 0; c < b.t - 1; ++c) {
			...
			if ((a[c + b.t] += b.am(c + 1, 2 * b[c], a, 2 * c + 1,
					d, b.t - c - 1)) >= b.DV) {
				a[c + b.t] -= b.DV;
				a[c + b.t + 1] = 1
			}
	...
	function C(a, b) {
		var c = a % this.DB, d = this.DB - c, e = (1 << d) - 1, f = Math
				.floor(a / this.DB), g = this.s << c & this.DM, h;
	...
	function u(a) {
		...
		var c = (1 << b) - 1, d, e = !1, f = "", g = this.t, h = this.DB
				- g * this.DB % b;
		if (g-- > 0) {
	...
	function h(a, b, c, d, e, f) {
		...
		while (--f >= 0) {
			var i = this[a] & 16383, j = this[a++] >> 14, k = h * i
					+ j * g;
	...
	function g(a, b, c, d, e, f) {
		...
		while (--f >= 0) {
			var i = this[a] & 32767, j = this[a++] >> 15, k = h * i
					+ j * g;
			i = g * i + ((k & 32767) << 15) + c[d]
					+ (e & 1073741823);
...
function encodePwd(pubkey,servertime,nonce,password){
